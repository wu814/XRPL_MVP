...
export async function POST(req: NextRequest): Promise<NextResponse<APIResponse<never>>> {
  ...
  if (fetchError || !request || request.receiver !== receiver) {
    return NextResponse.json({ success: false, message: "Request not found or unauthorized" }, { status: 403 });
  }
  ...
  return NextResponse.json({ success: true, message: `Friend request ${action}ed` }, { status: 200 });
}
