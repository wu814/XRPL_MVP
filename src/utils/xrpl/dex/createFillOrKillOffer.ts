...
export default async function createFillOrKillOffer(
  ...
  try {
    ...
    const response: TxResponse<OfferCreate> =
      await client.submitAndWait<OfferCreate>(signedTx.tx_blob);
    ...
    if (isTypedTransactionSuccessful(response)) {
      ...
      try {
        ...
        const modifiedOffers = affectedNodes.filter(
          (node: any) =>
            node.ModifiedNode && node.ModifiedNode.LedgerEntryType === "Offer",
        );

        const deletedOffers = affectedNodes.filter(
          (node: any) =>
        ...
        if (modifiedOffers.length > 0 || deletedOffers.length > 0) {
          ...
          const accountRootChanges = affectedNodes.filter(
            (node: any) =>
              node.ModifiedNode &&
          ...
          if (accountRootChanges.length > 0) {
            ...
            if (change.PreviousFields?.Balance && change.FinalFields?.Balance) {
              ...
              if (balanceChange !== 0) {
                ...
                message += `💰 Actual XRP received: ${dropsToXrp(actualAmountReceived)} XRP\n`;
              }
            }
          }
        } else {
    ...
    } else if (getTypedTransactionResult(response) === "tecKILLED") {
      console.log(
      ...
      message += `💱 Attempted to pay: ${
        typeof takerGets === "object"
          ? `${parseFloat(takerGets.value).toFixed(6)} ${takerGets.currency}`
          : `${dropsToXrp(takerGets as string).toFixed(6)} XRP`
      }\n`;
      message += `💱 Attempted to get: ${
        typeof takerPays === "object"
          ? `${parseFloat(takerPays.value).toFixed(6)} ${takerPays.currency}`
          : `${dropsToXrp(takerPays as string).toFixed(6)} XRP`
      }\n`;
      message += `📋 Transaction Hash: ${response.result.hash}\n`;
      message += `📋 Ledger Index: ${response.result.ledger_index}\n`;
      message += `📋 Transaction Result: ${getTypedTransactionResult(response)}\n`;
    ...
    return {
      success: false,
      message: `Fill-or-kill offer failed: ${error.message}`,
