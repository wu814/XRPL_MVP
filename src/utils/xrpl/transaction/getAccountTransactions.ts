...
interface AffectedNode {
  CreatedNode?: NFTokenPage | NFTokenOffer;
  ModifiedNode?: NFTokenPage | NFTokenOffer | RippleState | AccountRoot;
  DeletedNode?: NFTokenOffer;
}
...
function extractNFTTokenId(txData: TransactionData): string | null {
  ...
  for (const node of meta.AffectedNodes) {
    if (node.CreatedNode?.LedgerEntryType === "NFTokenPage") {
      const nftPage = node.CreatedNode as NFTokenPage;
      if (nftPage.NewFields?.NFTokens && nftPage.NewFields.NFTokens.length > 0) {
        return nftPage.NewFields.NFTokens[0].NFToken?.NFTokenID || null;
      }
    }
    if (node.ModifiedNode?.LedgerEntryType === "NFTokenPage") {
      const nftPage = node.ModifiedNode as NFTokenPage;
...
function extractNFTOfferPrice(txData: TransactionData, tx: any): string | null {
  ...
  for (const node of meta.AffectedNodes) {
    if (node.CreatedNode?.LedgerEntryType === "NFTokenOffer") {
      const offer = node.CreatedNode as NFTokenOffer;
    ...
    if (node.ModifiedNode?.LedgerEntryType === "NFTokenOffer") {
      const offer = node.ModifiedNode as NFTokenOffer;
      if (offer.FinalFields?.Amount) {
        return formatAmount(offer.FinalFields.Amount);
      }
      if (offer.PreviousFields?.Amount) {
  ...
  for (const node of meta.AffectedNodes) {
    ...
    // For XRP deposits
      ...
      if (!addedAssets.has("XRP") && xrpSent > 1000) {
        assetsDeposited.push({
          currency: "XRP",
          value: xrpl.dropsToXrp(xrpSent.toString()).toFixed(6),
        });
        addedAssets.add("XRP");
      }
    }
  ...
  for (const node of meta.AffectedNodes) {
    ...
    // For XRP withdrawals
      ...
      if (!addedAssets.has("XRP") && xrpReceived > 1000) {
        assetsWithdrawn.push({
          currency: "XRP",
          value: xrpl.dropsToXrp(xrpReceived.toString()).toFixed(6),
        });
        addedAssets.add("XRP");
      }
    }
...
export async function getAccountTransactions({ 
  ...
  const processedTransactions = (accountTx.result.transactions as TransactionData[]).map((txData: TransactionData): ProcessedTransaction | null => {
    try {
      ...
      console.warn("Error processing transaction:", error instanceof Error ? error.message : String(error));
      return null;
    }
