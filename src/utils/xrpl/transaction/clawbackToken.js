 ...
 */
export default async function clawbackTokens (issuerWallet, account, currency, amount) {
  try {
    await connectXrplClient();
    
    console.log(`🔹 Preparing to clawback ${amount} ${currency} from ${account}...`);
    ...
    console.log("📊 Issuer account info:", JSON.stringify(accountInfo.result.account_data, null, 2));
    ...
    console.log(`🔍 lsfAllowTrustLineClawback bit (position 16): ${relevantBit}`);
    ...
    if (!hasClawbackFlag) {
      ...
      console.log("⚠️ Warning: AllowTrustLineClawback flag check failed, but proceeding with clawback attempt anyway.");
      console.log("⚠️ If the transaction fails, please ensure you've set flag 16 (asfAllowTrustLineClawback) on your account.");
    } else {
    ...
    console.log(`🔍 Checking if ${account} has a trustline with ${issuerWallet.classicAddress} for ${currency}...`);
    ...
    console.log(`📊 Trustline info:`, JSON.stringify(accountLines.result, null, 2));
    
    // Find the specific trustline for the currency
    const trustline = accountLines.result.lines.find(line => line.currency === currency);
    
    if (!trustline) {
      throw new Error(`No trustline found for ${currency} between ${account} and ${issuerWallet.classicAddress}`);
    }
    
    console.log(`✅ Found trustline with balance: ${trustline.balance}`);
    
    if (parseFloat(trustline.balance) <= 0) {
      throw new Error(`Account has no ${currency} balance to claw back (current balance: ${trustline.balance})`);
    }
    ...
    console.log("📜 Prepared transaction:", JSON.stringify(preparedTx, null, 2));
    ...
    if (response.result.meta.TransactionResult === "tesSUCCESS") {
      console.log(`✅ Successfully clawed back ${amount} ${currency} from ${account}`);
      return response;
    } else {
