 ...
 */
export default async function createSellOffer(wallet, takerPays, takerGets, destinationTag = null) {
  try {
    ...
    console.log("📜 Prepared Sell OfferCreate TX:", JSON.stringify(offerCreateTx, null, 4));
    ...
    if (response.result.meta.TransactionResult === "tesSUCCESS") {
      ...
      console.log(`💱 Selling: ${typeof takerGets === 'object' ? `${takerGets.value} ${takerGets.currency}` : `${xrpl.dropsToXrp(takerGets)} XRP`}`);
      ...
      throw new Error(`Sell OfferCreate failed: ${response.result.meta.TransactionResult}`);
    }
  } catch (error) {
