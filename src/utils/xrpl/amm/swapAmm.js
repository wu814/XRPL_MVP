...
const swapInSpecificAMM = async (
  ...
  try {
    ...
    console.log(
      `🔄 AMM Swap: ${fromAmount} ${fromCurrency} → ${toCurrency} in pool ${ammAccount}`,
    );
    console.log(
      `⚡ Checking specific pool first, then comparing with other AMMs...`,
    );
    ...
    const targetRate = calculateSwapRate(
      targetAmmInfo,
      fromCurrency,
      toCurrency,
      fromAmount,
    ...
    console.log(
      `🎯 Target AMM Rate: ${targetRate.toFixed(6)} (Est. output: ${targetOutput.toFixed(6)} ${toCurrency})`,
    );
    ...
    if (pathfindingResult.success && pathfindingResult.bestPath) {
      ...
      const pathfindingOutput = parseFloat(
        pathfindingResult.bestPath.estimatedOutput,
      );

      console.log(
        `🔍 Best Alternative Rate: ${pathfindingRate.toFixed(6)} (Est. output: ${pathfindingOutput.toFixed(6)} ${toCurrency})`,
      );
      ...
      if (pathfindingRate > targetRate * improvementThreshold) {
        console.log(
          `✨ Found better rate! Switching from target pool to ${pathfindingResult.bestPath.type}`,
        );
        useTargetPool = false;
        ...
        console.log(
          `✅ Target pool has competitive rate, proceeding with original plan`,
        );
      }
    } else {
    ...
    if (useTargetPool) {
      ...
      console.log(
        `🌐 Executing via cross-currency payment for optimal routing`,
      );
      const {
        sendCrossCurrency,
      } = require("../transactionController/sendCrossCurrency");
...
const executeAmmSwap = async (
  ...
  try {
    ...
    if (response.result.meta.TransactionResult === "tesSUCCESS") {
      ...
      console.log(
        `   Effective Rate: ${(parseFloat(actualOutput) / parseFloat(fromAmount)).toFixed(6)}`,
      );
      ...
      throw new Error(
        `AMM swap failed: ${response.result.meta.TransactionResult}`,
      );
    }
  } catch (error) {
...
const calculateSwapRate = (ammInfo, fromCurrency, toCurrency, fromAmount) => {
  const currencyA =
    typeof ammInfo.amount === "string" ? "XRP" : ammInfo.amount.currency;
  const currencyB =
    typeof ammInfo.amount2 === "string" ? "XRP" : ammInfo.amount2.currency;
    ...
    for (const node of affectedNodes) {
      if (
        ...
        if (
          ...
          const diff =
            parseFloat(xrpl.dropsToXrp(finalFields.Balance)) -
        ...
        if (
          finalFields?.Balance?.currency === outputCurrency &&
          previousFields?.Balance?.currency === outputCurrency
        ) {
          const diff =
            parseFloat(finalFields.Balance.value) -
